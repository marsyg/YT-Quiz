...
export default function PlayerPage() {
  ...
  const [playlistData, setPlaylistData] = useState<{
    ...
    currentIndex: number;
  } | null>(null);// here PlaylistData is an object
        ...
        try {
          ...
          if (storedPlaylist) {
            ...
            const response = await fetchPlaylistItems(
              session.accessToken,
              'PLQnljOFTspQU2bFV-N5Ix-Qg9A3WAik7N',
            );
      ...
      <div className="lg:w-3/4">
        {videoId ? (
          ...
          <Card className="p-6 m-2">
            <CardContent >
              <h1>Choose a video from the playlist</h1>
              <Button variant="outline">Get Notes</Button>
            </CardContent>
          </Card>
        )}
        <div>
        <Card className="p-6">
            <CardContent>
              Click to understand the video better . 
             <Button variant="outline" onClick={() => router.push(`/notes?videoId=${videoId}`)}>Get Notes</Button>
            </CardContent>
          </Card>
        </div>
        <div className="mt-6">
